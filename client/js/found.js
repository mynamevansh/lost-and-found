document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadItems() {
        try {
            const response = await fetch('https://lost-and-found-9cwe.onrender.com/api/items/found');
            ...
            if (deleteBtn) {
                deleteBtn.addEventListener('click', async (e) => {
                    ...
                    if (confirm('Are you sure you want to delete this item?')) {
                        try {
                            const response = await fetch(`https://lost-and-found-9cwe.onrender.com/api/items/found/${item._id}`, {
                                method: 'DELETE',
                                headers: {
